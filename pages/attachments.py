...
from utils import current_user, flash, record_activity, redirect, save_upload, slugify, url_for
...
async def attachment_upload():
    ...
    await record_activity(subject_type, subject_id, user, "attached", filename=upload.filename)
...
async def attachment_download(attachment_id: int):
    attachment = await db.first(Attachment.select().where(Attachment.id == attachment_id))
    if attachment is None:
        redirect(url_for("dashboard"))
    return FileResponse(
        os.path.join(UPLOAD_FOLDER, attachment.stored_path),
...
async def attachment_delete(attachment_id: int):
    ...
    if attachment is not None:
        ...
        if os.path.exists(abs_path):
            os.remove(abs_path)
        await attachment.adelete_instance()
