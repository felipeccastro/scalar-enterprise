...
from models import CLIENT_STATUSES, Client, Task, db
...
async def clients_list():
    ...
    clients = await db.list(query.order_by(Client.created_at.desc()))
    return await render(
        "clients_list.html",
        clients=clients,
...
async def clients_create():
    ...
    await record_activity("client", client.id, user, "created")
...
async def client_detail(client_id: int):
    client = await db.first(Client.select().where(Client.id == client_id))
    if client is None:
    ...
    return await render(
        ...
        tasks=tasks,
        statuses=CLIENT_STATUSES,
        comments=await _load_comments("client", client.id),
        attachments=await _load_attachments("client", client.id),
        activity=await _load_activity("client", client.id),
    )
...
async def client_update(client_id: int):
    ...
    await client.asave()
    await record_activity("client", client.id, await current_user(), "updated")
    flash("Client updated.", "success")
    redirect(url_for("client_detail", client_id=client.id))


@app.route("/clients/{client_id:int}/archive", method="POST", name="client_archive")
async def client_archive(client_id: int):
    client = await db.first(Client.select().where(Client.id == client_id))
    if client is not None:
        client.archived_at = datetime.datetime.now()
        await client.asave()
        await record_activity("client", client.id, await current_user(), "archived")
...
async def client_restore(client_id: int):
    client = await db.first(Client.select().where(Client.id == client_id))
    if client is not None:
