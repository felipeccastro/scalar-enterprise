...
from models import Invite, PasswordReset, TeamMember, User, db, seed_demo_data
...
async def register_owner_submit():
    ...
    if await db.exists(User.select().where(User.email == email)):
        flash("That email is already in use.", "error")
        redirect(url_for("register_owner"))
    user = await User.acreate(name=name, email=email, password_hash=hash_password(password))
    await TeamMember.acreate(user=user, role="owner")
    await seed_demo_data(user)
    login_user(user)
...
async def login_form():
    if await current_user() is not None:
...
async def login_submit():
    ...
    if _login_ip_limit.hit(client_ip()) or _login_email_limit.hit(email or "unknown"):
        ...
        redirect(url_for("login"))
    try:
        user = await User.aget(User.email == email)
...
async def invite_teammate():
    ...
    await Invite.acreate(email=email, token=token, role="member", invited_by=user)
    invite_url = str(request.url).split("/invite", 1)[0] + url_for("accept_invite", token=token)
    try:
        await asyncio.to_thread(Mailer.send_invite, email=email, invite_url=invite_url, inviter_name=user.name)
...
async def accept_invite_form(token: str):
    try:
        invite = await Invite.aget((Invite.token == token) & (Invite.accepted == False))  # noqa: E712
    except Invite.DoesNotExist:
        flash("That invite link is invalid or has already been used.", "error")
        redirect(url_for("login"))
    return await render("accept_invite.html", invite=invite)


@app.route("/invite/{token}", method="POST", name="accept_invite_submit")
async def accept_invite_submit(token: str):
    try:
        invite = await Invite.aget((Invite.token == token) & (Invite.accepted == False))  # noqa: E712
    except Invite.DoesNotExist:
        flash("That invite link is invalid or has already been used.", "error")
    ...
    user = await User.acreate(name=name, email=invite.email, password_hash=hash_password(password))
    await TeamMember.acreate(user=user, role=invite.role)
...
async def forgot_password_form():
    return await render("forgot_password.html")
...
async def forgot_password_submit():
    ...
    if email and not _reset_email_limit.hit(email):
        try:
            ...
            user = None
        if user is not None:
            invalidate = PasswordReset.update(used=True).where(
                (PasswordReset.user == user) & (PasswordReset.used == False)  # noqa: E712
            )
            await invalidate.aexecute()
            token = secrets.token_urlsafe(32)
            expires_at = datetime.datetime.now() + datetime.timedelta(minutes=30)
            await PasswordReset.acreate(user=user, token=token, expires_at=expires_at)
            reset_url = str(request.url).split("/forgot-password", 1)[0] + url_for("reset_password", token=token)
            try:
                await asyncio.to_thread(Mailer.send_password_reset, email=email, reset_url=reset_url, ttl_minutes=30)
            except MailerError:
...
async def reset_password_form(token: str):
    return await render("reset_password.html", token=token)


@app.route("/reset-password/{token}", method="POST", name="reset_password_submit")
async def reset_password_submit(token: str):
    password = request.forms.get("password") or ""
    try:
        reset = await PasswordReset.aget(
    ...
    user = await reset.afetch(PasswordReset.user)
